...
- 全部 `[ ]` → 正常从第 1 步开始
- 全部 `[x]` → 输出「所有步骤已完成」，进入第 4 步直接输出汇总
- 混合 → 记录第一个 `[ ]` 的步骤编号，第 2 步初始化任务列表时已完成步骤直接设为 `completed`，第 3 步从第一个 `[ ]` 步骤开始执行
...
3. **步骤级止损**：执行过程中遇到以下任一情况，立即触发止损协议（见下方）：
...
处理方式：<小调整时说明调整内容并继续；大偏差时停止本步骤，在汇总「偏差记录」节标记，等待主 session 处理>
```

> Agent 模式无法弹交互，按以下规则自主决策：
> - **小偏差**（如行号偏移 ±5 行、方法名拼写变化）→ 调整后继续，在汇总中记录
> - **大偏差**（如方法签名不匹配、文件不存在、多处未列出的调用方）→ 停止本步骤，在汇总「偏差记录」中标记为「阻塞」，说明需要人工干预的具体原因
...
### 第 3.5 步：完成验收清单（Verification Checklist）

所有步骤执行完毕后，在输出汇总前，逐项核查以下清单并在汇总中附上结果：
...
| 4 | **范围外发现已显式声明** | 执行中发现的范围外问题，均已在对应步骤摘要「发现但未触碰」字段中记录 |
| 5 | **现有测试未被覆盖** | 未修改任何现有测试文件，或已在偏差记录中说明原因 |

> Agent 模式无法交互阻塞：检查项若有 ❌，**在汇总「偏差记录」中标记**，说明具体违反原因，由主 session 处理。
...
汇总必须包含：
- 变更文件列表（路径 + 变更类型 + 一句话说明）——供 forge-review 直接用作审查范围，必须完整准确
- 关键决策汇总
- 已知风险/建议检查点
- 未触碰声明（汇总自各步骤「发现但未触碰」字段，无则删除）
- 偏差记录（若有步骤与计划不符，说明原因）
- 验收清单结果（第 3.5 步各项的 ✅ / ❌ / N/A）

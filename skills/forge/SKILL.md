...
name: forge
description: >
  当用户描述功能需求、Bug 修复或任何功能改动时触发，例如"我想做 X"、"帮我加 X"、"改一下 X"、"我需要 X"、"实现 X"，
  或用户运行 /go、/onboard 命令时触发。
...
3. **若用户选「直接做」**：
   - 立即调用 forge-implement skill 执行编码
   - 编码完成后建议运行 `/review` 进行代码审查

4. **若用户选「走完整流程」**：
   - 调用 forge-clarify skill 开始多轮对话精化需求

...
### 第 2 步：批量提问（一次性收集所有关键信息）

带着第 1 步的探索结果，**一次性批量提问**，最多 4 个问题，覆盖：需求核心确认 + 边界 + 场景 + 可能的影响。
...
**盲区维度主动探测**（所有级别）：除了回应用户已描述的内容，从以下维度中选 1-2 个用户最可能遗漏的，主动加入提问：

| 维度 | 探测方向 |
|------|---------|
| 空状态 / 异常场景 | 数据为空、加载失败、网络异常时的处理方式是否明确？ |
| 权限 / 角色差异 | 不同角色（游客/普通用户/管理员）行为是否一致？ |
| 新旧数据兼容 | 已有数据在新逻辑下能否正常展示？是否需要迁移？ |
| 性能边界 | 数据量极大或极小时是否有特殊预期？ |
| 多端一致性 | 改动是否需要在其他端（iOS/桌面/Web）同步？ |
...
若用户选择「需要修改」或在 Other 中输入反馈，更新对应节后重新执行第 6 步自查，再展示草稿确认，**最多修改 1 轮**。
...
- **需求文档写完后由用户决定是否自动启动 /plan**：需求文档是 `/plan` 和 `/implement` 的契约基础，用户在第 7 步明确选择是否继续进行方案设计，由用户控制流程

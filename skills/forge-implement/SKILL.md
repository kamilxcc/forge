...
提取后在内部保持这三节内容，**Step 4.5 每步执行前都要对照检查**：本步改动是否违反任何设计决策或边界约定？是否触碰了风险点？若有冲突 → 停止执行，用 AskUserQuestion 说明冲突并询问用户。
...
- 可并行的步骤在第 4.5 步中**合并为单次 Agent 并发调用**，而非串行等待
...
2. **约束核对**：对照 Step 3 提取的执行约束基线，检查本步改动是否违反设计决策或边界约定，是否触碰风险点；有冲突 → 停止，AskUserQuestion 说明冲突
3. **理解步骤**：Read task.md 中本步骤声明的文件路径，**不做额外 Glob/Grep**（task.md 已由 /plan 定位好文件）
   - **禁止重读原则**：执行前检查「本轮已读文件列表」，若该文件**本轮已读且未被之前步骤修改** → 直接复用已有内容，不再 Read；若该文件已被前步修改 → 必须重新 Read 获取最新状态
...
| "这个文件刚才没改，重新 Read 一下更保险" | 防止信息过时 | 禁止重读原则：本轮已读且未被修改的文件直接复用，重复 Read 浪费 token；只有前步已修改过的文件才需要重新 Read |

...
阈值和规则以 `<plugin-root>/references/execution-mode-protocol.md` 为准，读取 task.md 中的 `recommended_mode` 字段（或手动数步骤数），**直接按推荐模式执行，不弹 AskUserQuestion**：

- **≤ 3 步** → inline，直接进入第 3 步
- **4–6 步** → 派发 forge-executor（Agent 模式，见下方）
- **> 6 步** → 派发 forge-executor，同时输出：
  ```
...
**inline 模式特有**（以下逻辑在 execution-core-protocol.md 中无对应，仅 inline 执行）：
...
所有步骤完成后，按 `<plugin-root>/references/structured-step-output.md` 的「汇总格式」输出完整汇总。

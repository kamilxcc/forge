...
description: >
  ...
  触发命令：/implement。
  前置条件：.forge-kb/.state/current-task.md 必须存在且 status=confirmed。
...
- `.forge-kb/.state/current-task.md`（执行计划，必须存在）
...
```
Read .forge-kb/.state/current-task.md
```

若文件不存在：
```
❌ 执行计划不存在（.forge-kb/.state/current-task.md 未找到）。
...
Read `features/<slug>.md`（从 current-task.md 的 `feature_doc` 字段获取路径）。
...
每步完成后，将 `.forge-kb/.state/current-task.md` 中对应步骤的 `- [ ]` 改为 `- [x]`。

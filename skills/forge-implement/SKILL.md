...
本 skill 文件位于 `<plugin-root>/skills/forge-implement/SKILL.md`，因此：
- plugin-root = 本文件向上**两级**目录
- 例：本文件路径为 `/Users/foo/forge-plugin/skills/forge-implement/SKILL.md` → `<plugin-root>` = `/Users/foo/forge-plugin`
...
2. **理解步骤**：Read task.md 中本步骤声明的文件路径，**不做额外 Glob/Grep**（task.md 已由 /plan 定位好文件）

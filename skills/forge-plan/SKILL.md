...
本 skill 文件位于 `<plugin-root>/skills/forge-plan/SKILL.md`，因此：
- plugin-root = 本文件向上**两级**目录
- 例：本文件路径为 `/Users/foo/forge-plugin/skills/forge-plan/SKILL.md` → `<plugin-root>` = `/Users/foo/forge-plugin`
...
2. Glob 定向搜索：`**/模块名/**/*.kt`、`**/相关类名*.kt`
3. Grep 搜索关键类名、接口名、方法签名
4. 结合 Grep 结果缩小候选文件范围

**终止条件**：达到预算上限，或已能列出每个任务步骤对应的候选文件 → 立即进入阶段 B。

产出：**候选文件列表**（路径 + 一句话说明为何候选）

---

#### 阶段 B：定向精读（Read，硬预算）

**目标**：读取阶段 0/A 选出的关键文件，为 task.md 每个步骤填入参考锚点。

预算（硬性上限）：
- Read 文件数：**≤ 6 个**
- 每文件读取行数：**≤ 150 行**（聚焦构造函数、公共方法签名、关键注释；无需读全文）

执行步骤：
1. 从候选文件列表中按相关性排序，优先读最核心的文件
2. 读取时聚焦目标：构造函数签名、公共方法名、关键注释，**不逐行阅读逻辑**
3. 读完一个文件后，立即更新对应步骤的参考锚点；若已能填满所有步骤，**立即停止**，不读剩余文件

**终止条件**：达到预算上限，或所有步骤的参考锚点已就绪 → 进入第 4 步。
...
- **跳过 KB 直接探索**：第 3 步必须先走阶段 0（KB 优先），KB 覆盖率 ≥70% 时直接进阶段 B，不得绕过；阶段 A/B 有硬性预算（Glob ≤5 / Grep ≤8 / Read ≤6 文件 ≤150 行），到达上限必须停止；基于已有信息出方案，找不到参考实现的步骤标注 ⚠️，而不是继续探索

...
**终止条件（目标驱动）**：每个实现单元的每个涉及维度都能对应到至少一个候选文件 → 立即进入阶段 B。

若某个维度搜索多轮后仍无候选文件 → 标记为 ❓ 探索盲区，**不继续搜索**，进入阶段 B 后直接升级为阻塞性问题。
...
**终止条件（目标驱动）**：所有实现单元的所有步骤都有了参考锚点 → 进入第 4 步。

若某文件读完仍无法确认参考锚点 → 在 task.md 该步骤标注 `⚠️ 无参考实现`，**不继续翻文件**，在第 4 步升级为阻塞性问题。

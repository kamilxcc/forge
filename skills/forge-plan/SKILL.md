...
#### 前置：探索面声明（Stage A/B 开始前必须完成）

基于需求文档，声明本次探索**必须覆盖的维度**，每个维度在 Stage A/B 中都必须产出至少一个候选文件。

**标准维度清单（按需勾选，不涉及的维度可划掉）**：
...
声明后，Stage A/B 结束时**每个维度必须有明确处置**，不允许无声消失：
...
- **跳过探索面声明**：第 3 步必须在 Stage A 开始前完成维度声明；不声明就直接探索，预算会被最熟悉的层（通常是数据层）吸走，导致导航层、容器约束等被遗漏；Stage A/B 结束后必须逐维度检查覆盖情况

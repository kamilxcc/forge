...
| 维度 | 说明 | 是否涉及 |
|------|------|---------|
| 数据层 | 数据模型、decode/encode、TYPE 常量 | □ |
| 渲染层 | Delegate、CardView、Adapter | □ |
| 导航层 | 页面打开方式、Router、Fragment/Activity 容器约束 | □ |
| 分发层 | transform、dispatcher、工厂方法 | □ |
| 公共接口 | 被多处依赖的接口、BaseClass、工具类 | □ |
| 持久化层 | Room Entity、SharedPreferences、序列化格式 | □ |
| 外部依赖 | 后端接口、跨团队模块 | □ |

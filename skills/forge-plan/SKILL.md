...
阶段 B 读完一个文件后，立即把该文件中 executor 需要参考的关键内容（接口签名、枚举常量、插入点附近代码，≤20 行）**直接内联写入 task.md 对应步骤的「做什么」字段**，而不是写到独立缓存文件。这样 task.md 是自包含的，implement 不需要再 Read 代码文件。
...
**「做什么」字段代码详细度原则**：
- **标准模式**（Fragment 骨架、object 单例、普通 data class、对称追加的枚举值）→ 只写字段名/方法名，**不展开完整实现**；executor 会自行照现有模式补全
- **项目特有业务逻辑**（schema URL、路由协议、跨模块 IOC 调用约定、非标准接口签名）→ 写出完整代码片段，不可省略
- **对称结构**（"参照 Step N，在 Y 模块做同样的事"）→ 写 `参照 Step N` 即可，**不重复展开**
- 判断依据：该内容是否能从现有代码直接推断？**能推断 → 省略；不能推断（项目约定/外部协议）→ 保留**
...
- **task.md 代码过度展开**：标准模式（Fragment 骨架、object 单例、对称枚举追加等）写完整实现，既增加 plan 阶段生成 token，也让 task.md 体积膨胀、executor 读上下文变慢；只有项目特有业务逻辑（schema/路由/IOC 调用）才需要完整片段；能从代码直接推断的 → 只写方法名/字段名

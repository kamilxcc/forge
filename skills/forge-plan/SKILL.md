...
**终止条件**：每个单元达到预算上限，或已能列出该单元各任务步骤的候选文件 → 立即进入阶段 B。

产出：**候选文件列表**（按单元分组，每个文件一句话说明为何候选）

---

#### 阶段 B：定向精读（Read，按单元分配预算）
...
> 💡 N 个实现单元的总 Read 上限 = N × 3 个文件，预算随需求复杂度自然伸缩，无需人为限制。

执行步骤：
1. 按单元逐一处理：从该单元候选文件列表中按相关性排序，优先读最核心的文件
2. 读取时聚焦目标：构造函数签名、公共方法名、关键注释，**不逐行阅读逻辑**
3. 读完一个文件后，立即更新对应步骤的参考锚点；若已能填满该单元所有步骤，**立即停止该单元的探索**，转向下一个单元

**终止条件**：所有单元的预算耗尽，或所有单元的参考锚点已就绪 → 进入第 4 步。
...
- **跳过 KB 直接探索**：第 3 步必须先走阶段 0（KB 优先），KB 覆盖率 ≥70% 时直接进阶段 B，不得绕过；阶段 A/B 的预算按实现单元计（每单元 Glob ≤3 / Grep ≤5 / Read ≤3 文件 ≤150 行），每个单元达到上限必须停止；基于已有信息出方案，找不到参考实现的步骤标注 ⚠️，而不是继续探索
